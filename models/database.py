...
def init_db():
    ...
    if cursor.fetchone()[0] == 0:
        books_data = [
            ...
            ("Karanlığın Sol Eli", "Ursula K. Le Guin", "Bilim Kurgu", 1969, "Çift cinsiyetli canlıların yaşadığı kış gezegeninde bir elçinin hikayesi."),
        ...
        for i, b in enumerate(books_data):
            ...
            cursor.execute('''INSERT OR IGNORE INTO books 
                           ...
                           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''', 
                           (b[0], b[1], isbn, b[2], b[3], b[4], cover_url, 5, 5))
